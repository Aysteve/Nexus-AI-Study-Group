...
import { SessionStatus, TranscriptEntry, VoiceName } from '../types.ts';
...
const useGeminiLive = (systemInstruction: string, voiceName: VoiceName, onMicVolumeChange?: (volume: number) => void) => {
        ...
        callbacks: {
          onopen: () => {
            ...
            scriptProcessorRef.current.onaudioprocess = (audioProcessingEvent) => {
              ...
              if (onMicVolumeChange) {
                  ...
                  onMicVolumeChange(Math.sqrt(sum / inputData.length));
              }
          ...
          onmessage: async (message: LiveServerMessage) => {
            ...
            if (base64Audio && outputAudioContextRef.current) {
                ...
                if (isRecording) recordedChunksRef.current.push(audioBuffer.getChannelData(0).slice(0));
  ...
  }, [systemInstruction, voiceName, status, isRecording, onMicVolumeChange, cleanup]);
  ...
  const toggleRecording = useCallback(() => setIsRecording(p => !p), []);
