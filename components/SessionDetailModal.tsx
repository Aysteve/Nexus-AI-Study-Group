  ...
  const generateSummary = useCallback(async () => {
    ...
    try {
        ...
        const response = await ai.models.generateContent({
            ...
            contents: `Please provide a detailed summary of the following study session transcript. Highlight key concepts, questions asked, and main conclusions. Format it neatly using markdown.\n\nTranscript:\n${session.transcript}`,
        });
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="w-full max-w-4xl bg-white dark:bg-gray-900 border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl flex flex-col h-[85vh] animate-fade-in">
        <header className="flex-shrink-0 flex items-start justify-between p-4 border-b border-slate-200 dark:border-white/10">
          <div>
            <h2 className="text-xl font-bold text-blue-600 dark:text-blue-400">{session.topic}</h2>
               ...
               <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="flex items-center gap-2">
                       ...
                       <div className="flex items-center bg-slate-200 dark:bg-white/10 rounded-full p-0.5">
                          {[1, 1.5, 2].map(rate => (
                              <button key={rate} onClick={() => handleRateChange(rate)} className={`px-2 py-0.5 text-xs rounded-full ${speechRate === rate ? 'bg-blue-600 text-white' : ''}`}>{rate}x</button>
                          ))}
                       </div>
                  </div>
              </div>
              <textarea readOnly value={session.transcript} className="flex-1 bg-transparent border border-slate-200 dark:border-white/10 rounded p-2 text-sm text-slate-600 dark:text-gray-300 w-full resize-none focus:outline-none" />
              <button onClick={() => downloadFile('transcript.txt', session.transcript)} className="mt-2 w-full flex items-center justify-center gap-2 bg-slate-200 dark:bg-white/10 hover:bg-slate-300 dark:hover:bg-white/20 py-2 rounded transition-colors"><Download size={16} /> Download Transcript</button>
            </div>
            <div className="md:w-1/2 flex flex-col bg-slate-50 dark:bg-black/30 rounded-lg p-4">
              ...
              <div className="flex-1 bg-transparent border border-slate-200 dark:border-white/10 rounded p-2 text-sm text-slate-600 dark:text-gray-300 overflow-y-auto">
                {isLoading && <div className="flex items-center justify-center h-full"><Loader2 className="animate-spin w-8 h-8 text-blue-600" /></div>}
                ...
                {!isLoading && !summary && !error && (
                  <div className="flex flex-col items-center justify-center h-full text-center">
                      <p className="text-slate-500 dark:text-gray-400">No summary generated.</p>
                      <button onClick={generateSummary} className="mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-full">Generate Summary</button>
                  </div>
                )}
              </div>
               {summary && !isLoading && (
                <button onClick={() => downloadFile('summary.txt', summary)} className="mt-2 w-full flex items-center justify-center gap-2 bg-slate-200 dark:bg-white/10 hover:bg-slate-300 dark:hover:bg-white/20 py-2 rounded transition-colors"><Download size={16} /> Download Summary</button>
              )}
            </div>
          </div>
        </div>
