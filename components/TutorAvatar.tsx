...
const TutorAvatar: React.FC<TutorAvatarProps> = ({ tutor, isSpeaking, isActive }) => {
  return (
    <div className="flex flex-col items-center gap-2 transition-all duration-300 transform" style={{ opacity: isActive ? 1 : 0.6, scale: isActive ? '1' : '0.9' }}>
      <div className={`relative rounded-full p-1 ${isSpeaking ? 'bg-indigo-500 animate-pulse' : ''}`}>
        <img
          src={tutor.avatarUrl}
          alt={tutor.name}
          className={`w-28 h-28 rounded-full border-4 object-cover transition-all duration-300 ${isActive ? 'border-indigo-400' : 'border-slate-300'}`}
        />
         {isSpeaking && (
            <div className="absolute bottom-0 right-0 w-6 h-6 bg-indigo-500 rounded-full border-2 border-white flex items-center justify-center">
